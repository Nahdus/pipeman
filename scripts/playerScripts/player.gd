...
export var gravity_max : float = 900
# ------------- #
...
func attackAnimationComplete() ->void:
...
func animate() ->void:
	...
	if velocity.x == 0 and is_on_floor():
#		print_debug($playerAnimation.animation)
		if get_input()["attack"]:
			attacking = true
			if $playerAnimation.animation != "attack":
				$playerAnimation.play("attack")
				$playerAnimation.connect("animation_finished",self,"attackAnimationComplete")
		elif !attacking:
